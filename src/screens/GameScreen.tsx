...
import { type GamePhase, type LevelInfo } from '../types';

interface GameScreenProps {
    children: React.ReactNode; // Puzzle se stále renderují jako děti v App.tsx
}
...
export const GameScreen: React.FC<GameScreenProps> = ({ children }) => {
    ...
    // Načteme vše potřebné z kontextu
    const {
        settings,
            ...
            >
                ...
                >
                    ...
                    <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                        ...
                        <button
                            onClick={() => {
                                addLog('Manual diagnostics run.', 'system');
                                play('click');
                            }}
                            className="mt-8 w-full border border-dashed border-slate-700 p-2 text-xs text-slate-500 hover:text-white flex items-center justify-center gap-2"
                        >
                    ...
                    <div className="p-6 border-t border-cyan-900/50 bg-black/40 space-y-3">
                        ...
                        <button
                            ...
                            className="w-full py-3 border border-red-900/50 text-red-500 font-mono font-bold hover:bg-red-900/20 transition-all text-xs"
                        >
                            {t('game_menu_exit')}
                        </button>
                    </div>
